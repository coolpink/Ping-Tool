 ...
 */
class cpWidgetFormInputMediaBrowser extends sfWidgetForm
{
   ...
   *          The url is relative to app_cp_media_browser_root_dir
  ...
  public function render($name, $value = null, $attributes = array(), $errors = array())
  {
    ...
    $dir = $value
         ? dirname($value)
         : sfConfig::get('app_cp_media_browser_root_dir').'/'.$this->getOption('dir');
    
    $url = $this->context->getRouting()->generate('cp_media_browser_select', array('dir' => $dir));

    $tag = $this->renderTag('input', $attributes)
           .$this->includeView()
           .$this->includeDelete();
    
    // Add javascripts and stylesheets as configure in app_cp_media_browser_assets_widget
  ...
  /**
   * Insert dependant javascripts and include a <script> with cpMediaBrowserWindowManager.addListerner
  ...
  protected function loadJavascript(array $params)
  {
    ...
    <script type="text/javascript">
      cpMediaBrowserWindowManager.addListerner({target: '{$params['id']}', url: '{$params['url']}'});
    </script>
EOF;
  }
  ...
  /**
   * Wraps a tag within a <span class="cp_media_browser_input_file"></span>
   * @param string $tag tag to wrap
   * @return string HTML string
   */
  protected function wrapTag($tag)
  {
    return '<span class="cp_media_browser_input_file">'.$tag.'</span>';
  }
}
